...
func (r *TodoItemPostgres) GetAllItems(userId int, listId int) ([]todo_app.Item, error) {
	...
	getAllItemsQuery := fmt.Sprintf(`SELECT tl.id, tl.title, tl.description, tl.done FROM %s tl INNER JOIN %s li on li.item_id = tl.id
...
func (r *TodoItemPostgres) GetItemById(userId int, itemId int) (todo_app.Item, error) {
	...
	getItemsByIdQuery := fmt.Sprintf(`SELECT tl.id, tl.title, tl.description, tl.done FROM %s tl INNER JOIN %s li on li.item_id = tl.id
						INNER JOIN %s ul on ul.list_id = li.list_id WHERE tl.id=$1 AND ul.user_id=$2`, 
						todoItemsTable, listsItemsTable, usersListsTable)
